...
def generate_today_signal() -> dict:
    ...
    # 获取数据
    csi300 = data_service.get_index_data('000300.SH')
    csi1000 = data_service.get_index_data('000852.SH')
    
    if csi300 is None or csi1000 is None:
        raise Exception("获取数据失败")
    
    # 初始化策略
    dmr_strategy = DMRStrategy(
        momentum_window=config.strategy.momentum_window,
        ma_window=config.strategy.ma_window
    )
    
    ml_model = MLRiskModel(
        trigger_threshold=config.ml.risk_trigger_threshold,
        release_threshold=config.ml.risk_release_threshold
    )
    
    dmr_ml_strategy = DMRMLStrategy(dmr_strategy, ml_model)
    
    # 生成信号
    signal_gen = SignalGenerator(dmr_ml_strategy)
    signal = signal_gen.generate_daily_signal(csi300, csi1000)
