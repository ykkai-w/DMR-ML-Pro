...
def render_admin_page():
    ...
    # 加载订阅者数据
    try:
        from subscription_service import load_subscribers, delete_subscriber, SUBSCRIPTION_FILE
        
        subscribers = load_subscribers()
        
        st.markdown(f"### 📊 订阅用户管理")
        st.info(f"📍 数据文件：`{SUBSCRIPTION_FILE}`")
...
def main():
    ...
    st.markdown("""
    <style>
    button[kind="secondary"][key="secret_admin_btn"] {
    ...
    button[kind="secondary"][key="secret_admin_btn"]:hover {
        background-color: transparent !important;
        border: none !important;
        color: #CCCCCC !important;
    }
    ...
    # 版权信息（点击©进入管理模式）
    col1, col2, col3, col4, col5 = st.columns([2, 3, 0.15, 2.85, 2])
    
    with col2:
        st.markdown("""
        <div style="text-align: right; color: #AAAAAA; font-size: 0.8rem; line-height: 2.5;">
            DMR-ML Pro v1.0-内测版 | 基于机器学习的双重动量轮动策略 |
        </div>
        """, unsafe_allow_html=True)
    
    with col3:
        # ©符号作为管理入口（点击进入）
        if st.button("©", key="secret_admin_btn", type="secondary", help=""):
            st.session_state.admin_mode = True
            st.rerun()
    
    with col4:
        st.markdown("""
        <div style="text-align: left; color: #AAAAAA; font-size: 0.8rem; line-height: 2.5;">
            2026 ykai-w
        </div>
        """, unsafe_allow_html=True)
