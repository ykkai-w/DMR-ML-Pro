...
def render_sidebar():
    ...
    with st.sidebar:
        # 隐藏的管理员入口（点击版权触发）
        if 'show_admin_entrance' not in st.session_state:
            st.session_state.show_admin_entrance = False
...
def main():
    ...
    # 点击计数器
    if 'footer_click_count' not in st.session_state:
        st.session_state.footer_click_count = 0
    ...
    with col2:
        if st.button("⚠️ 风险提示：本策略基于历史数据回测，过往业绩不代表未来表现。投资有风险，决策需谨慎。\n\nDMR-ML Pro v1.0-内测版 | © 2026 ykai-w", 
                     key="footer_btn", use_container_width=True):
            import time
            current_time = time.time()
            
            # 如果在3秒内连续点击
            if current_time - st.session_state.last_click_time < 3:
                st.session_state.footer_click_count += 1
            else:
                st.session_state.footer_click_count = 1
            ...
            # 连续点击5次进入管理模式
            if st.session_state.footer_click_count >= 5:
