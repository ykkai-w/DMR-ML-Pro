...
def main():
    ...
    # 版权信息 + 隐藏管理入口（©符号是可点击的隐形按钮）
    col1, col2, col3, col4, col5 = st.columns([3, 2, 0.2, 3, 3])
    
    with col2:
        st.markdown("""
        <div style="text-align: right; color: #AAAAAA; font-size: 0.8rem;">
    ...
    with col3:
        # ©符号作为隐形按钮（和背景色一样）
        if st.button("©", key="hidden_admin_gate", help=""):
            st.session_state.admin_mode = True
            st.rerun()
    
    with col4:
        st.markdown("""
        <div style="text-align: left; color: #AAAAAA; font-size: 0.8rem;">
            2026 ykai-w
        </div>
        """, unsafe_allow_html=True)
