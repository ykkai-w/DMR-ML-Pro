...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # 管理入口（未激活时显示）- 使用深蓝色小按钮（独立样式）
        if not st.session_state.admin_mode:
            # 用 form 包裹，可以精确控制样式
            with st.form(key="admin_form", clear_on_submit=False):
                st.markdown("""
                <style>
                /* 只针对管理员表单的按钮 */
                div[data-testid="stForm"][data-testid*="admin"] button,
                ...
                with col2:
                    submitted = st.form_submit_button("🔑", use_container_width=True, help="管理员入口")
                    if submitted:
