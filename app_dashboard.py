...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # 管理入口（未激活时显示）- 使用深色低调样式
        if not st.session_state.admin_mode:
            # 注入CSS：使用标记元素+相邻兄弟选择器精确定位管理按钮
            st.markdown("""
            <style>
            /* 管理按钮样式 - 深色低调 */
            #admin-btn-marker + div button {
            ...
            #admin-btn-marker + div button:hover {
                background: #383838 !important;
                border-color: #505050 !important;
                color: #aaaaaa !important;
                box-shadow: none !important;
            }
            #admin-btn-marker + div button:active,
            #admin-btn-marker + div button:focus {
                background: #333333 !important;
                box-shadow: none !important;
            }
            </style>
            <div id="admin-btn-marker" style="display:none;"></div>
            """, unsafe_allow_html=True)
